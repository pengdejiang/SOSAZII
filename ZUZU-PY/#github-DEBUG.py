...
file_paths = ["天津联通.txt", "山西联通.txt","安徽电信.txt", "河南联通.txt", "河南电信.txt", "福建电信.txt", "江苏电信.txt", "四川电信.txt", "重庆联通.txt", "重庆电信.txt","山东电信.txt","广东电信.txt","广西电信.txt","江西电信.txt","河北电信.txt","浙江电信.txt","湖南电信.txt","辽宁联通.txt","陕西电信.txt"]  #替换为实际的文件路径列表
...
with open('合并IP.txt', 'r', encoding='utf-8') as file, open('A-IP记录存档.txt', 'a', encoding='utf-8') as A-IP记录存档:    #####定义临时文件名

    for line in file:

        if re.search(pattern, line) and line.count(',') == 1:  # 如果行中有任意关键字而且行内只有一个逗号

         A-IP记录存档.write(line)  # 将该行写入输出文件 #####定义临时文件

for line in fileinput.input("A-IP记录存档.txt", inplace=True):  #打开文件，并对其进行关键词原地替换    
...
file_paths = ["天津联通.txt", "山西联通.txt","安徽电信.txt", "河南联通.txt", "河南电信.txt", "福建电信.txt", "江苏电信.txt", "四川电信.txt", "重庆联通.txt", "重庆电信.txt","山东电信.txt","广东电信.txt","广西电信.txt","江西电信.txt","河北电信.txt","浙江电信.txt","湖南电信.txt","辽宁联通.txt","陕西电信.txt","JIEXI.txt"]  #替换为实际的文件路径列表
