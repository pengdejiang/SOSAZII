...
file_paths = ["天津联通.txt", "山西联通.txt","安徽电信.txt", "河南联通.txt", "河南电信.txt", "贵州电信.txt", "福建电信.txt", "江苏电信.txt", "四川电信.txt", "重庆联通.txt", "重庆电信.txt","山东电信.txt","广东电信.txt","广西电信.txt","江西电信.txt","河北电信.txt","浙江电信.txt","湖南电信.txt","辽宁联通.txt","陕西电信.txt"]  #替换为实际的文件路径列表
...
with open('合并IP.txt', 'r', encoding='utf-8') as file, open('IP_save.txt', 'a', encoding='utf-8') as IP_save:   
    ...
    for line in file:
        ...
        if re.search(pattern, line) and line.count(',') == 1:  # 如果行中有任意关键字而且行内只有一个逗号
         ...
         IP_save.write(line)  # 将该行写入输出文件 #####定义临时文件

for line in fileinput.input('IP_save.txt', inplace=True):  #读取文件-'合并IP.txt'，对"IP_save.txt"追加写入读取的所有IP行   
...
#复制一份跳转文件出来
with open('IP_save.txt', 'r', encoding='utf-8') as file1, open('IP_save-jump.txt', 'w', encoding='utf-8') as file2:
    # 逐行读取文件1.txt的内容
    for line in file1:
        # 将读取到的内容写入文件2.txt
        file2.write(line)
...
# 读取文件IP_save-jump.txt，处理并写入文件IP_save.txt
with open('IP_save-jump.txt', 'r', encoding='utf-8') as file1, open('IP_save.txt', 'w', encoding='utf-8') as file2:
    # 创建一个集合来存储非重复的URL部分
    unique_parts = set()
    ...
    for line in file1:
        ...
        line = line.strip()
        if not line or ',' not in line:  # 如果是空行或没有逗号的行
            file2.write(line + '\n')  # 写入原行到文件IP_save.txt
...
file_paths = ["天津联通.txt", "山西联通.txt","安徽电信.txt", "河南联通.txt", "河南电信.txt",  "贵州电信.txt","福建电信.txt", "江苏电信.txt", "四川电信.txt", "重庆联通.txt", "重庆电信.txt","山东电信.txt","广东电信.txt","广西电信.txt","江西电信.txt","河北电信.txt","浙江电信.txt","湖南电信.txt","辽宁联通.txt","陕西电信.txt","JIEXI.txt"]  #替换为实际的文件路径列表
...
with open("SOSAZI-over.txt",'w', encoding='utf-8') as file:   #新建输出文件
    ...
    for line in lines:  #查找每一行
		#如果IP当中包含下面数字
        if line.find("//27.10") != -1 or line.find("//220.176.218") != -1 or line.find("//171.221.129") != -1 or line.find("//118.122.111") != -1 or line.find("//27.191.71") != -1 or line.find("//175.11.171") != -1 or line.find("//220.176.218") != -1 or line.find("//117.43.80") != -1 or line.find("//182.139.152") != -1 or line.find("//118.122.111") != -1 or line.find("//118.112.60") != -1 or line.find("//27.191.71") != -1 or line.find("//122.232.188") != -1 or line.find("//125.71.44") != -1 or line.find("//122.234.77") != -1 or line.find("//122.232.188") != -1 or line.find("//14.105.105.35") != -1 or line.find("//27.191.71.248") != -1 or line.find("//144.255.40.160") != -1 or line.find("//140.250.221") != -1 or line.find("//223.242.146") != -1 or line.find("//182.139.215") != -1 or line.find("//182.150.168") != -1 or line.find("//120.32.11") != -1 or line.find("//113.86.204.209") != -1 or line.find("//110.185.44") != -1 or line.find("///61.157.92.168") != -1 or line.find("//220.175.145") != -1 or line.find("//117.25.38") != -1 or line.find("//125.82.171") != -1 or line.find("//117.12.148") != -1 or line.find("//183.5.92") != -1 or line.find("//117.66.231") != -1 or line.find("//36.47.83") != -1 or line.find("//115.221.95") != -1 or line.find("//113.120.108") != -1 or line.find("//115.193.167") != -1 or line.find("//117.28.112") != -1 or line.find("//117.25.38") != -1 or line.find("//117.67.169") != -1 or line.find("//221.15.251") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1 or line.find("//+") != -1:    #在or后面添加更多需要删除的//xxx地址-注意冒号结尾-中间没有冒号
